     ...
     $(document).ready(function() {
        ...
        var bootMenu = {
            ...
            showDropDown: function(event) { //show dropdown function
                var _this = $(this);
                var dropdownMenu = _this.find('.dropdown-menu').first();
            
                // ใช้ clearTimeout และ setTimeout เพื่อป้องกันการปิด sub-menu ก่อนเวลา
                if (event.type === 'mouseenter' && bootMenu.$navbar.hasClass('navbar-hover')) {
                    clearTimeout(_this.data('timeout'));
                    dropdownMenu.addClass('show').removeClass('hide');
            ...
            setDropAnimation: function(_this) { //animation function for dropdown menu
                ...
                // เพิ่มคลาส animated แบบไม่มี delay
                dropdown_menu.css({
                    'animation-duration': '150ms', // ลดระยะเวลา animation
                    'transition-duration': '150ms' // ลดระยะเวลา transition
                }).removeClass('animated').addClass('animated ' + animation_value);
            },
    ...
    // ---------- for Delete Class "wrap-card" When Screen 1199px
    
    window.addEventListener('resize', function() {
    
        const wrapCard = document.querySelector('ul.nav.navbar-nav');
    
        if (window.innerWidth <= 1199) {
    
            if (wrapCard && wrapCard.classList.contains('wrap-card')) {
    
                wrapCard.classList.remove('wrap-card');
    ...
    // เรียกใช้ครั้งแรกเพื่อเช็คขนาดจอเมื่อโหลดหน้า
